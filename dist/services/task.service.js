...
export const taskMake = async ({ data, user_id }) => {
...
export const viewATask = async (task_id, user_id) => {
...
export const aTaskUpdate = async ({ task_id, user_id, data }) => {
    const task = await taskRepo.findOne({
        where: {
            id: task_id,
            user: {
...
export const aTaskDelete = async (task_id, user_id) => {
    const task = await taskRepo.findOne({
        where: {
            id: task_id,
            user: {
...
export const userTasksGet = async (user_id, filters) => {
    const where = { user: { id: user_id } };
    if (filters.status)
...
export const cardSummery = async (user_id) => {
    const tasks = await taskRepo.find({
        where: {
            user: { id: user_id }
        }
